...
def convert_smoltalk(example):
    ...
    return {"prompt": question, "target": routed_model}
...
def convert_eval_data(item, task_type):
    ...
    return {"prompt": question.strip(), "target": routed_model}
...
def main():
    ...
    save_jsonl("router_classification/data/train.jsonl", train)
    save_jsonl("router_classification/data/dev.jsonl", dev)
    ...
    save_jsonl("router_classification/data/eval.jsonl", eval_data)

    print(f"\nTotal eval: {len(eval_data)}")
    for task, count in Counter(x['target'] for x in eval_data).items():
        print(f"  {task}: {count}")

    print("\nDone! Saved to router_classification/data/")
